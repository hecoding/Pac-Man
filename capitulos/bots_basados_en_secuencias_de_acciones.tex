...
Una derivación gramatical válida podría ser: ``\textbf{\texttt{U R D L}}'', que podría verse como un autómata con la forma de la Figura~\ref{fig:img:automata-jflap} \cite{jflapPage}:

\begin{figure}[h]
\centering
\includegraphics[width=8cm]{jflapo}
\caption{Autómata correspondiente al código \texttt{U R D L}}
\label{fig:img:automata-jflap}
\end{figure}
...
Para dicha prueba, introdujimos un nuevo estado de huida (similar a un estado trampa, pero con una vía de salida), en el que se entraba en caso de encontrarse algún fantasma (comestible) por debajo de un radio de peligro (ver Figura~\ref{fig:img:prueba-automata}). 
