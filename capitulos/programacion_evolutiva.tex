...
\item \textbf{Operadores de cruce}: se encargan de cruzar los genotipos de los individuos seleccionados. El cruce se realiza entre dos individuos previamente seleccionados denominados padres que se cruzan y generan dos individuos nuevos denominados hijos. Al igual que los mecanismos de selección, existen muchos operadores  para cruzar los genotipos, pero estos operadores son dependientes de la codificación del genotipo.

Si el genotipo es una cadena de enteros (codificación más común) el método de cruce más genérico es el cruce monopunto. Consiste en seleccionar aleatoriamente una posición idéntica en ambos genotipos de los padres (punto de corte) y generar dos hijos que conserven el mismo genotipo que su padre hasta la posición escogida y, a continuación, intercambiar la otra parte con la del otro padre, tal como se muestra en la Figura~\ref{fig:img:cruce-monopunto}.

\begin{figure}[h]
\centering
\includegraphics[width=0.7\textwidth]{operador-cruce}
\caption{Ejemplo de cruce monopunto.}
\label{fig:img:cruce-monopunto}
\end{figure}
...
\label{fig:img:arboles-genotipo-fenotipo}
\end{figure}
...
El operador de cruce más usado en programación genética, denominado cruce de subárboles, consiste en elegir aleatoriamente en cada padre un punto de cruce (un nodo) y el subárbol que forma es intercambiado con el subárbol del otro padre, como se ven en la Figura~\ref{fig:img:cruce-subarboles}.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{cruce-subarboles}
\caption{Operador de cruce de subárboles \cite{tsang2000eddie}.}
\label{fig:img:cruce-subarboles}
\end{figure}
...
\end{equation}
donde $\bmod$ es la función de módulo entero. Esta fórmula devuelve un número que es la posición de la producción a seleccionar del símbolo no terminal que está siendo expandido \cite{o2012grammatical}. Se puede ver un ejemplo en la Figura~\ref{fig:img:expandir-codon}.
...
Normalmente (como en el ejemplo anterior de inversión en bolsa) existen varios óptimos de Pareto, es decir, un conjunto de soluciones que dominan al resto pero que entre ellas no se dominan. A este conjunto de soluciones se le denomina \textit{frente de Pareto} (ver Figura~\ref{fig:img:pareto}).
